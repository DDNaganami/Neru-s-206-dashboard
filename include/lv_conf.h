...
// ------------------------------------------------------------
// 用途(点数是**分辨率派生**的,见 lib/themetool/ui_theme.h 末尾那张表):
//   48 号 —— 480×480 屏上转速/速度的**大数字**(表盘正上方)
//   18 号 —— 480×480 屏上的单位(km/h、rpm)与副表数字(水温/进气温度)
//   24 号 —— **240×240 屏**上的大数字(48 × theme_scale()=0.5)
//   10 号 —— **240×240 屏**上的单位与副表数字(18 × 0.5 = 9,取现成的 10)
//   14 号 —— LVGL 默认字体,留着给将来的小字
// ★ 48/18 与 24/10 是**同一份主题的两个分辨率档**,不是两套主题:
//   240 那块验证板(微雪 DualEye-Touch-LCD-1.28)与 480 的最终屏共用 theme.json。
// 代价:每个字号是一份点阵数据,会占 Flash(24 号 + 10 号 ≈ 47KB)。
//   当前 app 槽 1MB、固件占六成出头,余量足够;
//   若以后紧了,优先砍 48 号 —— 那是最终 2.8" 屏才用的,现在实配的是 240。
