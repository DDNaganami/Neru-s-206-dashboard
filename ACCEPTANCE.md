...
- [ ] 实车 K 线 OBD：010C/0105/**010F**/**010D** 持续更新，断线 3 秒回退假数据
      ★ **车那一侧验过一半**（2026-09-17，蓝牙 ELM327 + 手机 App 走 EOBD）：
      转速/水温都能实时读到，并且发现 ECU 还支持**进气温度(010F)**。
      ⚠️ 那次记的"车速也能读到"**不算数** —— 手机 App 显示的"车速"很可能
      来自 **GPS**（转速/水温只能是 OBD，车速几乎所有 App 都默认走 GPS）。
      所以 010D 要**单独验**：到车上直接用终端发 `010D` 看返回，
      或者在车库/没卫星锁定的地方看读数是否仍然合理。
