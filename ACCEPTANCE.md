...
- [x] 宿主机单元测试全绿（python -m platformio test -e native，当前 **92 例**，
      2 例需环境变量否则跳过）；网页端另有 4 个 Node 检查脚本
      （`test-face-stages.js` 205 项、`test-image-blob-build.js` 164 项、
...
- [x] **每屏只被自己那一路驱动**（可执行定义 + 两端测试）
      ...
      **实测证据**（pcpreview 落帧，8 张表情各一个颜色）：
      ...
      同一瞬间两块表显示**不同的**状态；左屏整段只在 红→黄→绿 之间变（纯转速），
      右屏整段只在 蓝→白(惊喜)→紫→橙 之间变（纯车速），两边从不同步。
...
- [x] **表情阶段表在固件与网页两端一致**：
      ...
      `test-face-stages.js` **解析 face_stages.h** 后逐字段对账（205 项断言）。
...
- [ ] 实车核对表情分档阈值：现在只有两路阈值 ——
      转速 2500/4500/6000（左屏）、车速 30/90（右屏）。
      到车上按这个看一眼：怠速轰到 3000 转左屏该变"巡航"、6000 转该变"红区"；
      60 km/h 右屏该是"巡航"、100 以上该是"运动"。
      不合适就改 `expression.cpp` 顶部那几个常数，别忘了同步
      `face_stages.h` 的阶段表（Node 对账测试会提醒）。
