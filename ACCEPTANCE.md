...
- [x] 宿主机单元测试全绿（python -m platformio test -e native，当前 **86 例**，
      2 例需环境变量否则跳过）；网页端另有 3 个 Node 检查脚本
      （`test-face-stages.js` 121 项、`test-image-blob-build.js` 160 项、
...
- [x] **图片资源（放 flash 分区，改图不用重编译固件）**：
      ...
      （见下面那条待办里的串口输出）。
- [x] **图片角色支持双屏两套、每屏 8 张**：`ImageRole` 现在 18 个在用 ——
      背景 + 9/10 保留不用 + **左屏 8 个状态（转速表）** + **右屏 8 个状态（速度表）**，
      状态是 常态/眨眼/巡航/运动/红区/惊喜/冷车/过热（见 `lib/dashcore/expression.h`）。
      **屏与表的对应按法系车来（标致 206 实车）：左屏 = 转速表，右屏 = 速度表**，
      水温表在转速表上。别按"左车速右转速"的日德习惯改回去。
      新状态从 **11** 开始编号（不是 9）—— 9/10 是保留编号，
      复用会让别人已导出的 image.bin 静默变成另一个角色。
...
- [x] **表情阶段表（低/中/高 × 转速/水温/速度）在固件与网页两端一致**：
      ...
      `test-face-stages.js` **解析 face_stages.h** 后逐字段对账（121 项断言）。
      理由：表情导入页的"阶段模拟"是用户刷图前唯一能看到的证据，
      网页那份和固件那份不一致，预览就是在骗人。
- [x] **缺图降级链**：8 个状态里只导入一部分也能正常显示 ——
...
- [x] **JS 打包器 ↔ 固件解析器的往返一致性测试**（`tools/theme-editor/test-image-roundtrip.ps1`）：
      Node 侧 160 项断言 + 用 JS 打镜像（覆盖全部角色）+ 让
...
3. **128 KB → 1 MB 的直接原因**：图片资源。去掉开机图后需要的是
   **1 背景 + 两屏各 8 个状态的表情**；240×240 时 16 张表情要 2.7 MB、
