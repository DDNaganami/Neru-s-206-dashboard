...
- [x] **每屏只被自己那一路驱动**（可执行定义 + 两端测试）
      ...
      **实测证据**（pcpreview 落帧，8 张表情各一个颜色）：
      ...
      两边从不同步。
- [ ] **「弧压在背景图之上」目前没有像素级覆盖**（外部复审指出，已确认）
      弧带(半径 181..205)与测试夹具的 240×240 底图(离圆心最远 169.7 在角上)**不重叠**,
      所以 check-preview-frame.js 里那条"弧带点"验的其实是"这一圈有东西",
      不是"弧盖在底图上"。夹具能验的是:底图画上了、表情透明边透出底图、弧带上有弧。
      要真验这条,把 make-test-blob.js 的底图改成 480×480
      (并相应把朝向检查的"缺口"判定改成"既不是底图色也不是主题底色")。
      当前判断:不上板前不必做,但它是一条**被文档声称、实际没验**的性质,记在这里。
...
- [ ] **把图片画到真屏上**（`dash_ui.cpp` 的图层已接好，**未上真屏**）。
      宿主机预览（pcpreview）已经逐像素验证过：表情图压在最上层、透明通道生效、
      缺图按降级链退、数字读数位置正确；
      ★ 注意"背景图在弧**下**"这条**不**在已验清单里（见上面那条未覆盖项：
      夹具底图只有 240×240，与弧带 181..205 不重叠）。真屏上换一张**满屏**底图
      就能看出这条；"弧在底图之上"目前只由 `dash_ui.cpp` 的创建顺序保证。
      另外"真屏驱动取像素的路径"仍要等实驱动写完才算数。
