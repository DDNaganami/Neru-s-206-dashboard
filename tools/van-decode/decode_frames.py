...
结论(2026-09-19):
  ...
  · 该字段全程**单调平滑**:停车 16(=8.0km/h) → 加速 → 巡航平台 92(=46.0km/h)
    → 末端 162(=81.0km/h)。
  · ★ **标度 0.5km/h 一位是推测**:没有地面真值(车速表读数),是按"巡航 ~46km/h"
    这个常识猜的。同样**量程也没定**(本段最高只到 81,看不出上限)。
  · 所以下一步必须做一次**标定跑**:分别稳在 20/40/60/80km/h 各几秒,
    一次就能同时定标度和量程。
