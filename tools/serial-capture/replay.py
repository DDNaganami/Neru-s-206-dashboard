...
怎么判断成功:
  ...
  · 之后每 5 秒那行 `SRC speed=... rpm=... coolant=... intake=...` 里
    对应的字段从 `sim` 变成 `van`。
  本项目的**假定**车速帧是 IDEN `0x824`、`data[2..3]` 大端、×0.01 得 km/h
  (见 lib/dashcore/van_source.h 的 kSpeedIden/kSpeedOffset/kSpeedScale),
  所以默认那帧 `18 F8 27 10 00 00 00` 算出来正好是 **100.0 km/h** ——
  看到 `v=100.0km/h` 且 `speed=van`,就说明整条链通了。
  (实车 IDEN 是否就是 0x824 必须实测确认,见 ACCEPTANCE 的实车必验清单。)
...
def main():
    ...
    print('判据:上面的 SRC 行里 speed 应该从 sim 变成 van,'
          '数值应稳定在 100.0km/h(默认帧算出来的就是 100.00)')
    if n == 0:
