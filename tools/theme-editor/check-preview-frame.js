 ...
 *   画面是 480×480,测试底图 240×240 居中(偏移 120),表情 100×100 居中(偏移 190)。
 ...
 *
 * ★ 一个必须说清的限制:测试底图只有 240×240(离圆心最远 169.7 在角上),
 *   而弧带在 181..205 —— **两者不重叠**,所以"弧压在背景图之上"这条
 *   用当前夹具**测不到**。夹具能测的是:底图画上了(四边中点为底色)、
 *   表情透明边透出底图、弧带上确实有弧。要真验"弧压底图",
 *   把 make-test-blob.js 的底图改成 480×480 即可(会连带改朝向检查的底色判定)。
 *
...
function main() {
  ...
  //   这里要证的是"这一圈有东西(不是底色、不是纯黑)"。
  // ★ 它**证不了"弧压在背景图之上"**:测试底图 240×240 离圆心最远 169.7,
  //   与弧带 181..205 不重叠。"弧在上层"这点由表情透明边那一项间接佐证
  //   (表情图压在底图上、透出底色);要直接验得换 480×480 的测试底图。
  {
    ...
    checks.push({
      name: "弧带点(半径 200 处有弧,非底色/非纯黑)", x: 240 + d, y: 240 + d,
      got, expect: ARC_TRACK_OVER_BG,
      ok: !isBg && !isBlack && lum < 0x40 * 3      // 暗色但不是纯黑
    });
  }

  // --- 3. 表盘朝向(契约:满量程弧的缺口在正下方) ---
  //
  // ★ 这条是踩过坑才加的:两个编辑器画弧时多减了 90°(`(d-90)`),
  ...
  //     沿弧带半径 200 扫一圈,正下方(90°)必须是**缺口**(背景色),
  //     正上方(270°)必须有弧(轨道或点亮色)。
  ...
  //   边界像素会被裁剪/半亮,读出来不可靠。
  {
    ...
    // 缺口:应当是屏幕底色(主题 bg 0x141414 → 565 后 #101410)
    // 有弧:轨道或点亮色,总之**不是**底色
    const bottomIsBg = dist(bottom, { r: 0x10, g: 0x14, b: 0x10 }) <= 6;
    const topIsBg = dist(top, { r: 0x10, g: 0x14, b: 0x10 }) <= 6;
    checks.push({
      name: "表盘朝向(缺口在正下方)", x: 240, y: 440,
      got: bottom, expect: { r: 0x10, g: 0x14, b: 0x10 },
      ok: bottomIsBg,
      text: "6 点钟 " + hex(bottom) + (bottomIsBg ? " = 缺口 ✓" : " ≠ 缺口(说明表被转过了)")
    });
    checks.push({
      name: "表盘朝向(正上方有弧)", x: 240, y: 40,
      got: top, expect: { r: 0x18, g: 0x18, b: 0x18 },
      ok: !topIsBg,
      text: "12 点钟 " + hex(top) + (topIsBg ? " = 底色(说明表被转过了)" : " = 有弧 ✓")
    });
  }
