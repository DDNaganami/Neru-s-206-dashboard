...
(function (root, factory) {
  ...
  // ★ 保留编号 2 / 9 / 10 / 11 / 16 **不在这里出现、也不复用**:
  //   2 是当年的开机帧(开机画面已改成程序化扫表);
  //   9/10 是当年的左右屏开机图;11/16 是当年的左右屏"眨眼"图(眨眼状态已删除)。
  //   复用它们会让别人已导出的 image.bin 里那几张图突然变成别的表情,而且不报错。
  //   新角色从 11 往上接、跳过这几个号。
  //
  // 一套表情 7 张(状态见 lib/dashcore/expression.h 的 Face),顺序就是槽位顺序:
  //   Idle 常态 / Cruise 巡航 / Sport 运动 /
  //   Redline 红区 / Surprise 惊喜 / Cold 冷车 / Hot 过热
  var ROLE = {
    Background: 1,
    BootFrame: 2,   // 保留:编号不进下拉框,只为了让"2 被占用"这件事看得见
    // 左屏(转速表)
    FaceIdle: 3, FaceRedline: 4, FaceSurprise: 5,
    FaceCruise: 12, FaceSport: 13, FaceCold: 14, FaceHot: 15,
    // 右屏(速度表)
    FaceIdleR: 6, FaceRedlineR: 7, FaceSurpriseR: 8,
    FaceCruiseR: 17, FaceSportR: 18, FaceColdR: 19, FaceHotR: 20
  };
  var ROLE_NAMES = {
    1: "表盘背景", 2: "开机动画帧（保留不用）",
