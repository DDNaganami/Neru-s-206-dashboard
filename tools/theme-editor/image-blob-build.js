...
(function (root, factory) {
  ...
  var TARGETS = {
    ...
    s3: {
      id: "s3",
      // ★ 2026-09-24：改成**双** 2.8C（车主口径）——左右两块是**同型号同分辨率**的
      //   480×480 圆屏，各由一块 ESP32-S3 N16R8 驱动。所以：
      //     · 左屏 = 转速表 + 水温，右屏 = 速度表 + 进气温度（法系车口径，见 ROLE 那段）
      //     · 左右屏的差别**靠用途（role）表达**（左屏表情 / 右屏表情本来就分开）
      //     · 两份屏的内容相同 ⇒ **同一份 image.bin 与同一份 theme.json 两块板都刷**
      //     · **预算按单块板算**（每块板各有自己的 image 分区 8MB）——
      //       页面的"占用百分比"含义**没变**，不是两块加起来
      label: "ESP32-S3 N16R8(16MB flash) + 双 2.8C 圆屏（最终板）",
      chip: "esp32s3",
      ...
      //   除了分区大小,把三件最容易踩的事写进去:
      //     ① 屏是圆的(四角不可见,见 asset-spec.js 的 circleSafeSide);
      //     ② 这是**两块**屏 + 左右屏各自的含义(装反/做错屏不会报错,只会画错表);
      //     ③ 两块板刷**同一份** image.bin,而预算按**单块板**算(不是两块相加)。
      //   ★ 这一段是**纯文本**（页面用 textContent 塞进一行 hint，不渲染 markdown）
      //     ⇒ 用「」和逗号断句，**不要**写 **加粗** 那种标记（会原样显示成星号）。
      hint: "双 2.8C 最终板:两块同型号同分辨率的 480×480 圆屏(ST7701S)," +
            "可视圆 Ø70.13mm,四角不可见(矩形内容 ≤336)。" +
            "左屏 = 转速表 + 水温,右屏 = 速度表 + 进气温度;" +
            "左右屏靠用途区分(左屏表情 / 右屏表情)。" +
            "两块板刷同一份 image.bin:image 分区各 8MB," +
            "占用按单块板算(不是两块相加),整套表情 + 480×480 背景都放得下"
    },
  ...
  // ★ 默认目标板 = **2.8C（最终板）**（2026-09-24 车主口径变更：默认就改成
  //   "双 2.8C 最终版"）。以前是 "classic"，于是页面一打开就是经典板的
  //   1MB 分区与 480 屏的旧口径 —— 人在"最终板"的语境下看到的是**别块板**的
  //   规格，做的图照着错的数走（经典板的口径在下面 classic 那一档仍然有效）。
  //   ★ 这个值**不落盘**：页面的目标板下拉框只存在内存里（localStorage 里只有
  //     配色 THEME_KEY），所以改默认值立刻就生效，没有"旧记录挡住新默认值"的问题。
  var DEFAULT_TARGET = "s3";
