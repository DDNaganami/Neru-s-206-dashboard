...
(function (root, factory) {
  ...
  //     · 画布上限   —— "会不会盖住内圈副弧"（跟着**屏**走：480 档 320 / 240 档 160）
  ...
  function prepareAsset(src, sw, sh, opts) {
    ...
    if (wantAlpha) {
      ...
      if (!hasAlpha) {
        ...
        //   （它就是不透明方块），只是会把底下的弧线挡掉一块。
        //   拒绝它会把"我就想铺满"的人挡在门外；而警告能说清后果。
        out.warnings.push("这个用途**必须带 alpha**（它要叠在弧线上面），" +
                          "但这张图整张都不透明 ⇒ 屏上会盖住底下的弧线。" +
  ...
  function checkUpload(fileName, mime, roleId, targetId) {
    ...
    if (box.alpha === "required" && f.alpha === "none") {
      ...
      res.errors.push(f.label + " **没有 alpha 通道**，而这个用途（" +
                      box.role.label + "）必须带 alpha —— 它要叠在弧线上面，" +
                      "不透明的方图会把弧盖掉一块。请改用带透明背景的 PNG。");
    }
