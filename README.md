...
板间链路已从"有线 UART"改为**无线 ESP-NOW 为主**（有线那一档**停用但一行不删**，见 `ARCHITECTURE.md` §8.3）：两块板同信道 6、不连 AP、`WIFI_PS_NONE`，地址自配置（广播 HELLO → 学到对端 MAC → 转单播 → 存 NVS）。实测**持续满载 20,000 帧零丢包**。仍未达标两项：`gap_max` 105~120ms（契约 100ms）、p99 39~47ms（契约 20ms）—— 其下界是**发端主循环被抢占 ~90~100ms**（不是空中），因此把 TICK 的**发送**搬离主循环是下一步（可行性已验证，见 `ACCEPTANCE.md` 补记四）。
...
- **2026-09-27** ★★ **显示与链路各自定案一件**：
  ...
  **② 无线链路实测**：持续满载 **20,000 帧零丢包**；`gap_max` 105~120ms、p99 39~47ms 仍未达契约，而下界是**发端主循环被抢占 ~90~100ms**（258 个窗口里 52% 超 100ms）。已加 `stage=`（长圈归因）与 `txprobe=`（高优先级 20ms 定时器实际节拍）两个读数，**证明抢占不是关中断/关 cache** ⇒ 把 TICK 发送搬离主循环可行。
  还修掉三处**只在真链路上才暴露**的缺陷：收环永久死锁（空环时把可用空间算成 `N-H`，H 飘过 240 就再也绕不回去）、"一个包里塞多帧而接收端按一包一帧判"（丢包 69% 的真因）、测量信封漏进车辆数据（屏上冒 `coolant=9.0C`，逐字节对上了信封布局）。
...
1. ★ **(a) 把 TICK 的发送搬离主循环**（可行性已验证：高优先级任务躲得开那 100ms 抢占）—— 目标是让 `gap_max` 与 p99 落回契约。**注意**：调度改动可能饿着显示相关的任务，而**横纹只有眼睛能判**，所以这一步要在能看屏幕的时候做。
