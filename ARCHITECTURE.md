...
- **决定性的未知数是微雪板的自由脚数**：板载屏把那 20 根吃在板上、不占排针，wiki 能引出的只有 12PIN 口的 **GPIO0**（唯一空脚，且是 BOOT/strapping）+ D-/D+（19/20，原生 USB）+ TXD/RXD（43/44，板载 CH343P 的刷写/日志口），I2C 口那两根（7/15）明写**不能**当普通 GPIO ⇒ 主板 2 根、从板 1 根：**纸面够，但每根都有代价**（BOOT 键 / 原生 USB / 刷写日志口），得按原理图定 —— 第 1 条待确认。
- ✓ PINOUT.md 开头那句反向说法**已改正**（2026-09-21，commit `543c151`）：现为「八线(OPI) PSRAM 固定占 GPIO33-37 ⇒ 33-37 不可用；只有四线(Quad)才不占」，屏 SPI 原排 35/36/37 的那几行随之改「**脚位待定**」（等**微雪 ESP32-S3-LCD-2.8C 原理图**，即下面待确认 #1），PURCHASE 同口径 —— 与本表一致，本表按**被占**算。

**待确认（owner 欠）**
1. 微雪 2.8C 的**自由 GPIO 数 + PSRAM 大小**（主板 ≥2、从板 ≥1）—— wiki 写 S3R8/8MB/16MB，还需**原理图**复核，并确认面板那 20 根占在哪几脚、有没有能抢回来的；
2. 机械三项：**表玻→表盘纸净厚度 / 台阶面→表盘纸深度 / 表盘后方净空**，外加**孔的真实最小内径（到 0.1 mm）**；
3. Dwin 多出来的 **~3.4 mm**（76.48 − 73.03 = 3.45）在哪条边（FPC/IC 台阶）—— 决定怎么摆进 Ø89；
4. 表仓中间那道塑料桥能不能打掉 —— **只对上面已否掉的"一块宽条屏"有意义**，两条已定的路都不需要它。
