...
- **屏与表的对应按法系车来（标致 206 实车）**：
  ...
  **涨幅方向也相反**：转速弧从 start 端起涨；水温弧 `reverse=1`（从 end 端起涨 = 镜像），
  因为"下方半圆"默认只会从右端开始亮，而水温表该从左端起涨。
...
- 表情由 face_stages.h + expression.cpp 状态机驱动，**每屏一套、各看各的表**：
  ...
  · **右屏（速度表）只看车速**：常态(<30) / 巡航(≥30) / 运动(≥90) / 惊喜(急加速瞬态)
    车速三档**还没有实车数据**，暂按 城区/快速路/高速 取 30/90。
...
- 阶段表贯穿固件与网页：`lib/dashcore/face_stages.h` 是唯一事实来源，
  10 条用例（转速 4 档 + 车速 3 档 + 水温 3 档），**每行同时给出左右两屏的期望**，
