...
**分工**：A 板（主）挂 VAN 收发器只用 **1 根 GPIO**（只听，只用 RO/RX），渲染**左盘**（转速 + 水温）并把
整车状态**广播**给 B；B 板（从）不接任何 VAN 硬件，收 UART 上的状态，渲染**右盘**（车速 + 进气温度）。
**链路**：115200 8N1、**单向**（A TX → B RX）+ GND、3.3V；复用**已经实现的回放行格式**
`VAN 824 18 F8 27 10 00 00 00` ⇒ 从板**零新协议**（`van_replay` 那条路本来就在解它）。
...
| 背光 PWM | 1 | 1 |
| **每板小计** | **21** | **29** |
| 主板 +VAN RX +链路 TX；从板 +链路 RX | 主 23 / 从 22 | 主 31 / 从 30 |
