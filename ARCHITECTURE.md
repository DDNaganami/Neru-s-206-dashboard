...
- **图片资源也是运行时可换的**（`lib/themetool/image_blob.*`）：图片放在
  ...
  角色编号：1 = 背景，2 = 开机帧（不用），**9/10 保留编号不复用**（曾是开机图），
  **11/16 也是保留编号**（曾是左右屏的"眨眼"图，眨眼已删除），
  3/4/5 = 左屏常态/红区/惊喜，6/7/8 = 右屏常态/红区/惊喜，
  **12..15 = 左屏巡航/运动/冷车/过热，17..20 = 右屏同四张**。
  新角色从 11 往上接：老用户手里的 image.bin 里 9/10/11/16 可能是当年的
  开机帧或眨眼图，复用它们会让那几张图突然变成别的表情，而且不报错。
  表情图用 RGB565A8（带 alpha 平面，能叠在弧上），背景用 RGB565。
  存储预算：一张 240×240 表情 RGB565A8 = 169KB，一张 480×480 背景 RGB565 = 450KB ——
  1MB 的 image 分区装不下"两屏各 7 张 + 全屏背景"（≈3.3MB），
...
- 表情由 face_stages.h + expression.cpp 状态机驱动，一共 **7 个状态**：
  常态 Idle / 巡航 Cruise / 运动 Sport / 红区 Redline /
  惊喜 Surprise / 冷车 Cold / 过热 Hot。
  **每个状态一张可导入的差分图**（左屏 7 张 + 右屏 7 张，见下）
  · 曾经有第 8 个状态 Blink（眨眼），**已删除**：它只跟时间有关，
    与车速/转速/水温都无关 —— 说不出"什么工况下会眨"，也没法在阶段模拟里体现；
    用图片表情时它本来也几乎看不出来（没导入眨眼图就退到常态那张）。
  · 稳态**只由数据决定，与时间无关**：test_face_is_time_independent_when_idle
    连跑 20 秒断言表情不变 —— 这条同时防止有人再把"定时器驱动的表情"加回来。
- 三路数据各有"低/中/高"，表情按优先级取最该被看到的那个：
  红区(转速≥6000) > 过热(水温≥105) > 冷车(水温<70) > 运动(速度≥90 或转速≥4500)
  > 巡航(速度≥30 或转速≥2500) > 常态；惊喜(急加速)是**瞬态**，压在稳态之上。
...
测试（宿主机，不烧板）：
- test/test_dashcore/ 为 native 单元测试（Unity，当前 88 例，2 例需环境变量否则跳过），
  ...
  多源回退、表情状态机（7 状态 + 三路数据的分档与优先级 + 稳态与时间无关）、
  **表情阶段表**（低/中/高 × 转速/水温/速度 = 9 条，逐条断言 face_update 的输出；
  检查降级链一定能退到常态；**检查每组只变自己那一维** ——
  点"速度·中"时转速表不能跟着动，这是用户试用时抓出来的规则）
