...
* **周期 2000 ms**（`kPanelGuardPeriodMs`）。一次回读实测 **~400µs**（100kHz I2C，见 §13.8.4）
  ⇒ 摊到 2 秒里可以忽略。它在 `dash_display_poll()` 里按时间片跑，**没到点就是几次整数比较**。
* 最坏发现窗口 = **一个周期（2 s）**；实测两次注入都是 **~0.67 s**（见 13.9.5 的表）。
* ★ **连续 3 次**检查都异常 ⇒ 才**请求一次**自动重初始化，且之后有 **60 s 冷却期**。
  **不许**做成周期性重初始化 ✗ —— 那会让屏**定期闪**，而闪屏本身就是仪表盘上的故障现象。
* ★ 自动重初始化的**执行时刻**在 `dash_display_poll()` 开头（主循环里那个"可以慢一拍"的安全时刻），
  不在 I2C 对账那一刻 —— 重跑 41 步 + 重发一帧要 ~200ms（实测），不该把那一拍拉长。

### 13.9.4 日志字样（**判据就用这几行，改代码先改这一节**）

```
panelguard: exio rd=0x04 shadow=0x05 -> rewritten     ← ① 抓到了并按影子修回来了
panelguard: backlight rd=0 want=512 -> rewritten      ← ② 背光那一路（独立）
panelguard: exio rd err (no ack) -> retry next        ← ③ 读失败（不算被改写）
panelguard: anomaly x3 -> auto reinit                 ← 连续 3 次 ⇒ 请求一次自动重初始化
...
`rd` 在涨 = 守护在跑（每 2 秒 +1）；`fix` ≠ 0 = 真发生过"位被改写并且已经修回来"；
`bl` = 背光被重设的次数；`anom` = 发现不一致的次数（含已修的）。
...
**② 故意注入故障**（本单最有价值的一步；注入什么、怎么注入见 13.9.6）：

| 次 | 注入时刻 | 守护抓到并修回来的时刻 | 发现耗时 | 原文 |
|---|---|---|---|---|
| #1 | `[14.15s]` | `[14.82s]` | **0.67 s** | `inject: exio out 0x05 -> 0x04 (bit0, 影子保持 0x05) —— 等守护抓它` → `panelguard: exio rd=0x04 shadow=0x05 -> rewritten` |
| #2 | `[ 6.15s]` | `[ 6.82s]` | **0.67 s** | 同上（逐字相同的两行） |
| #3 | `[10.15s]` | `[10.82s]` | **0.67 s** | 同上 |

★ 三次都落在**同一个相位**上（注入点距上一次周期检查 ~0.67s）⇒ 这是"边界驱动"的正常表现，
**不是**每次都恰好 0.67s；最坏情况就是**一个周期 2 s**。
★ 注入期间与恢复之后，「`rgb: vsync=… (+54/s) wrap=… timeout=0`」与每秒一行
`206 dash ok` **一秒都没断** —— 与"**驱动计数全正常而玻璃黑了**"那次事故的签名**逐字相同**
（那正是这条路径被认定是最可能诱因的理由）。
★ 车主当场在屏前：**屏闪了一下（约 0.7 秒的白/黑）然后自己回来了** ——
这就是"守护把一次面板复位救回来"在人眼里的样子。
...
* 注入点：`src/dash_display_rgb.cpp` 的 `dash_panel_guard_fault_inject()`，门是
  **`-DPANEL_GUARD_FAULT_INJECT=1`**；串口字符是 **`i`**（判据在 `lib/dashcore/serial_cmd.h`）。
...
* ★★ **默认构建里这一段是空函数**（宏默认 0）：`i` 会被当成普通字符交回回放路径
  ...
  带 `-DPANEL_GUARD_FAULT_INJECT=1` 的镜像 Flash = **914,319 B**（87.2%），
  去掉那一行之后 **914,119 B** —— 差 **200 B**，正是那一段注入代码本身。
* ★ 安全前提（复现时照做）：① 注入必须在**守护已生效**的固件上做；② 回滚镜像随手可用；
  ③ **一次只注入一次**、不许连续折腾；④ 两秒内没自愈 ⇒ **立刻烧回**并保留现场；
  ⑤ 测完把临时注入路径**删掉**（本次就是这么做的）。
