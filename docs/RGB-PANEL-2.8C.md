...
| `flush` | ≈20/s | ≈88/s（**8 块** × 7.3 整屏） | 整屏 = **8 个"60 行"块**（＝ 40 元素行；LVGL 按**字节**算行高，见 §12.1 的 25-01 更正） |
...
* 那一笔的大小 = **一次 LVGL flush 的数据量** = **绘制缓冲的全部字节**；
* 上一版绘制缓冲是 **40 个元素行 ⇒ 缓冲 57.6KB**（= 480×40×**3B**，见下面那段
  "★ 25-01 更正"），实测 `blit_max ≈ 3011~33739µs` ✗；
* 它**没法再摊平**（必须等 LVGL 把这块缓冲收回去才能 `flush_ready` ⇒ 不能跨圈拖着拷 ✗），
  所以只能**把它变小**：本轮把绘制缓冲改成 **16 个元素行 ⇒ 缓冲 23.04KB** ✓。

> ★★ **2026-09-24 更正（25-01）：这一节原先写的"40 行 ⇒ 38.4KB / 16 行 ⇒ 15.36KB"
> 是错的** —— 那两个数是按 **2 字节/像素 × 元素行数** 算的，而
> `static lv_color_t draw_buf[480 * LINES]` 的**元素是 `lv_color_t`（LVGL 9 里 3 字节）**
> ⇒ 缓冲字节数 = `480 × LINES × 3`：
>
> | 档 | 元素行数 | **缓冲字节数（真值）** | LVGL 实际一块 = 多少行 RGB565 |
> |---|---|---|---|
> | 上一版 | 40 | **57,600 B（57.6KB）** | **60 行** = 57,600 ÷ (480×2) |
> | 本轮 | 16 | **23,040 B（23.04KB）** | **24 行** |
>
> 而 LVGL 的 PARTIAL 模式是按**字节**算行高的
> （`lv_display.c` 的 `lv_display_set_buffers_with_stride()`：`h = buf_size / stride`，
> `stride` = 480×2B）⇒ 一次 flush 真正搬的是 **24 行 = 23,040 B**。
> **旁证（同一份代码里的两个数自洽）**：RAM 从 40→16 元素行省下的正是
> `24 × 480 × 3B = 34,560 B = 34.5KB`（§12.3 表里"省 34.5KB"那一格），
> 而真机每秒行里打出来的 `fmax=480x24` 也正是 **24 行** —— 三处互相印证。
> ★ 那一行日志里的错数**已经改掉**（2026-09-24，只改打印出来的数、行为零改动）：
> 旧 `十六行(15360B)` ⇒ 新 **`24行(23040B)`**，两个数**全部现算**
> （`sizeof(draw_buf)` / `sizeof(lv_color_t)` / 显示格式每像素字节数），
> 并把**六项编译期事实**一次打进**同一行**（把"到底几字节"交给**编译器**回答，
> 不靠读文件推理；机型/配置换一代，这行自己就变）：
>
> ```
> rgb: 编译期事实 LV_COLOR_DEPTH=16 lv_color_t=3B draw_buf=23040B 元素行=16 flush≤24行(每行960B);单笔>bounce 18KB 旧口径不成立
> ```
>
> ★ 折成**一行**是刻意的（不是省字）：两条 `dash_logf` 的格式串 + 调用点要多花 ~160B flash，
> 一行只比原来的那一行多 **+88 B**（原始三条数与拆分见 ACCEPTANCE 当天条目 ⑨补）。
> ★★ **这行新口径随"下次烧写"生效**：本轮**没有**为了它单独烧板（它只是日志文本），
> 板上当前那份固件打出来的仍是旧数（`16行(15360B)`）—— 见到它时按 24 行 / 23,040 B 读。

> ★★ **2026-09-24 追加（25-02）：`LV_COLOR_DEPTH` 与 `sizeof(lv_color_t)` 不是一回事**
> —— 仓库里这两处看起来"互相矛盾"，其实**都对**，它们说的是两个不同的量：
>
> | 量 | 值 | 出处（可复核） | 它管什么 |
> |---|---|---|---|
> | `LV_COLOR_DEPTH` | **16** | `include/lv_conf.h:10`（注释：设备 16 位，只有 pcpreview 用 `-DLV_COLOR_DEPTH=32` 覆盖） | **显示格式** ⇒ `LV_COLOR_FORMAT_DEFAULT` = RGB565 ⇒ **2 字节/像素** |
> | `sizeof(lv_color_t)` | **3** | LVGL 9.3 的 `include/lvgl/draw/lv_color.h` 里那个 `{uint8_t blue; uint8_t green; uint8_t red;}` 结构（**与 `LV_COLOR_DEPTH` 无关**，LVGL 9 的 `lv_color_t` 恒为 3 字节） | 我们那个 `draw_buf[]` **数组元素**的大小 |
>
> 两者**各司其职**：数组按 3B/元素开出来（`2 字节对齐`那条注释也因此成立，
> `src/dash_display_rgb.cpp:1340`），而 LVGL 按**字节**用它、按**显示格式**算 stride。
> 把这两个"字节数"混成一个，就是原来那行日志算出 15360 的原因。
> ★ 已按任务书"只记录、不改配置"：`lv_conf.h` **一个字节没动** ✓。
>
> ★ **`fmax` 到底量的是什么**（把含义钉死，免得再用错）：
> `fmax` 是**一次 flush 回调收到的矩形**。LVGL 9.3 在回调**之前**就把脏区切成子块了
> —— `lv_refr.c` 的 `get_max_row()`：
> `max_row = (buf_act->data_size - overhead) / stride`（`overhead` 对 RGB565 = 0），
> 紧接着那个 `for(row = inv_a.y1; row + max_row - 1 <= inv_a.y2; row += max_row)` 循环
> 逐块渲染、逐块回调 ⇒ **一次 flush 的矩形不会超过"缓冲字节数 ÷ stride"**。
> 本构建 = `23040 ÷ 960` = **24 行**，与实测 `fmax=480x24` 逐字吻合 ✓。
> ⇒ 这里**不能**反过来推"flush 行数 = 数组元素行数"（那是 16）；**也不能**说
> "flush 可以大于绘制缓冲"（LVGL 会先切块）。正确的等式只有一个：
> **`flush 行 = 缓冲字节数 ÷ (宽 × 显示格式每像素字节数)`**。
...
★ **按真值重算这句前提**（原写的"15.36KB < 19.2KB ⇒ 能被吸收"用的是错数）：
这一笔**实际是 23.04KB，比 19.2KB 还大**（40 元素行那档是 57.6KB，更吸收不了）。
⇒ "**一笔的字节数小于两块 bounce 的总量**"这条**简化的**判据**并不成立** ✗；
真正成立的是**实测**（不是账算出来的）：`copy_max` 33,739µs → **1,332µs**、
`refresh` 从 192~211ms（±16ms）收到 **199~201ms（±1ms）**，而且这一笔的**耗时**
（≈1.3ms）已经落进"两块 bounce 能撑住的那段时间"里 —— 车主后来报的观感也是
按这个方向走的。⇒ **这条前提的正确写法是"按实测"，不要再用"23.04KB vs 19.2KB"去推** ✓。
代价：整屏刷新从 **8 块变 20 块**（480÷60 → 480÷24，每块固定开销变大）；
稳态是局部刷新 ⇒ 几乎无感 ✓，
而且**内部 SRAM 反而省了 34.5KB**（RAM 48.1% → **37.6%**，`heap` 183KB → **217KB** ✓
—— 这笔账与"24 元素行 × 480 × 3B"逐字节对得上，也是上面那张表的旁证）。
...
| 指标 | 40 元素行绘制缓冲（上一版；块 = **60 行 / 57.6KB**） | **16 元素行（交付档；块 = 24 行 / 23.04KB）** |
|---|---|---|
| **`blit_max`**（当次脏区那一笔 = **整块**绘制缓冲） | ~3011~33739µs ✗（57.6KB） | **1325~1332µs** ✓（≈1.33ms，**23,040B**） |
...
★★ **2026-09-24 深夜更新状态**（当晚又出了一次**同签名**的黑屏：车主吃完饭回来发现屏黑了、
固件一直活着 —— 全文见 `ACCEPTANCE.md` 当晚那一节）：

| # | 机制 | 支持它的观察 | **最新状态** |
|---|---|---|---|
| **A** | **方波段把 I2C 时钟从 100kHz 提到 400kHz**（这是本次自检**新引入**的变量）⇒ 万一有一个字节被打错，而那颗 TCA9554 的输出寄存器里**同时挂着 LCD_RST(EXIO1)/LCD_CS(EXIO3)** ⇒ 一次**面板复位**，而面板复位**不会自己回来** ⇒ **黑屏 + 驱动计数全正常**（与现场完全对得上） | 三次自检里前两次都没黑屏、第三次才出现 ⇒ 与"概率性"相容；但同一段代码已跑过约 3 万次事务，**没有直接证据** | **✓ 已删除（关闭）**：方波段也用 **100kHz**（与点屏时同一个档），源码里 `Wire.setClock` 一个都没有。★ 删掉之后当晚**又**黑了一次（那次没跑自检）⇒ 这条路径**已经不能解释**那次事故，正式按"已排除"记 |
| **B** | **③ 持续拉高 2 秒**：蜂鸣器**连续**通电 2 秒（① 是 200ms 级短脉冲）⇒ 3.3V 轨被拉低一下 ⇒ **面板**（ST7701 内部寄存器/电荷泵）掉状态，而 **ESP32 活着** | 黑屏第一次出现，正好是 ③④ 这两段**第一次**上板（之前两版固件只有 ①②） | ★★ **正在现场测试中（2026-09-24 深夜起）**：蜂鸣器已**静音**（`mute: 1`，NVS 保存）⇒ 那条支路**不再通电**。**若静音期间不再黑屏 ⇒ 机制 B 的嫌疑显著上升**（但"不再出现"不是证明：也可能是概率）。★ 新的取证手段见 §13.8.6（开机那行 **复位原因**）—— 机制 B 若成立，某次复位应当是 `BROWNOUT` |

★ **恢复办法（已实测过两次）**：**复位一次**（或重新烧写默认固件）—— 面板会重跑 41 步初始化，
黑屏即恢复。★ **2026-09-24 新增第二条路**：串口命令 **`r`**（见 §13.9）——
它做的正是"重跑 41 步 + 重发当前画面"，**但不用按物理 RST、也不会重启 MCU**。
当晚回烧默认固件 + 复位后的原始行：`rgb: RGB565 480x480 pclk=15000000Hz …已就绪`、
`rgb: 双framebuffer num_fbs=2 …`、`rgb: vsync=27(+27/s) … timeout=0`、`206 dash ok` ✓。

★ 注意：13.6 的结论（**有源**）**不能**用来在 A 与 B 之间选一个 —— 两者都与"有源蜂鸣器 + 黑屏"
相容。要分开它们，需要的是**两条新仪表**（都已落地）：① **复位原因**（§13.8.6，
`BROWNOUT` ⇒ 电压跌落 ⇒ B）；② **面板健康守护**（§13.9，抓的是"扩展器的复位位被改写"⇒ A 那类）。
另一个仍然值钱的信息是"**黑屏发生在哪一段**"（② 方波段 ⇒ A；③ 持续拉高那 2 秒 ⇒ B）——
下次若要复跑自检，**先记这一条**。

#### 13.8.6 ★ 新仪表：复位原因 + 复位累计次数（2026-09-24 深夜，**最便宜的一次取证**）

**做什么**：开机打一行（紧跟 `206 dash ok` 之后）

```
boot: reason=POWERON n=3 (raw=1) up=218ms tc=32.3C
```

| 字段 | 含义 | 备注 |
|---|---|---|
| `reason` | `esp_reset_reason()` 的名字 | `POWERON`=上电/插线；`EXT`=板上 RST 键；`SW`=软件复位；`PANIC`/`WDT`=崩了；**`BROWNOUT`=电压跌落** |
| `n` | **上电/复位累计次数**（NVS 命名空间 `dash`、键 `bootn`） | 掉电不丢。"这块板自己重启过几次"只有它记着（车上没有 USB 主机） |
| `raw` | 上面那个枚举的**数值** | 万一名字与这一版 IDF 对不上，原值还能查表 |
| `up` | 打到这一行时已运行多少 ms | 用来判断"是不是刚上电就重启" |
| `tc` | 片上温度传感器的**粗读**（±10°C 级） | 只够看"热不热"；车载温度那条风险的一个随手读数 |

★★ **判据（业主/我自己要看的那一条）**：**如果屏又黑了，而之后某次复位原因是 `BROWNOUT`
⇒ 机制 B 基本坐实**（"电压被拉低到 brownout 门限以下"是芯片自己的记录，不是我们的推测）。
反之：**`BROWNOUT` 一次都没出现，也不等于 B 不成立** ——
若电压只是"跌到面板掉状态、但没跌到 ESP32 的 brownout 门限"，这一行**什么都不会报**。
⇒ 它是一条**单侧**的判据（能证实、不能证伪），这一点必须说清楚。

★ 顺带一条：`n` **不是**"黑屏次数"（软件感知不到玻璃，见 §13.9 那条边界）。
...
## 13.9 ★ 面板健康守护 + 现场恢复命令 `r`（2026-09-24 深夜，"**仪表盘必须常亮**"的落地）

**起因** = 业主原话（屏从厂里回来、准备上实车时）：

> "屏幕回来了，**上实车的时候可不能这样，这毕竟是仪表盘，要常亮的**。"

⇒ 黑屏**不是观感问题，是功能性失效**（车速/转速/告警/灯全看不到）。
产品要求与三层防线写在 `ARCHITECTURE.md`「显示约定」**§3.5**；这里写**实现、判据、与上板实测**。

### 13.9.1 代码在哪（三层各一处）

| 层 | 文件 | 说明 |
|---|---|---|
| 判据层（**宿主机可测**） | `lib/dashcore/panel_guard.{h,cpp}` | 周期/对账规则/计数/连续异常判据。**输出与时钟全是注入的回调**（与 `buzzer_exio` 同一条纪律）⇒ 一个 I2C/Arduino/LEDC 符号都不引用 |
| 设备接线 | `src/dash_display_rgb.cpp` | 四个注入回调：读/写 TCA9554 输出寄存器（与蜂鸣器那条"回读自证"同一套时序）、读/写 LEDC 占空、面板重初始化 |
| 命令与挂载 | `src/main.cpp` + `lib/dashcore/serial_cmd.h` | `r` 命令的**判据**抽成纯函数（`src/` 编不进 native，见那个文件的说明）；诊断页那一格由 `sys_inputs_build()` 填 |

### 13.9.2 它守什么、**守不到什么**（边界，必须写在最显眼处）

| | 内容 |
|---|---|
| **守得到（①）** | 那颗 TCA9554 的**输出寄存器**与影子 `g_exio_out` 不一致 —— 也就是 `LCD_RST`(EXIO1)/`LCD_CS`(EXIO3) 被意外改写的**最可能那条黑屏路径**。动作：**按影子重写**（不是读-改-写、更不是接受读回来的值）+ 计数 + 一行日志 |
| **守得到（②）** | 背光 LEDC 的当前占空比与设定值不符 —— **另一条独立的黑屏路径**（玻璃黑了但面板是好的）。动作：重设 + 计数 + 一行日志 |
| **守得到（③）** | 回读**失败**（无应答）—— 单独计数、单独一行日志，**不算**"被改写"（总线一时不应答与寄存器真被改了是两件事） |
| **★ 守不到（必须说清）** | **"玻璃到底黑了没有"软件无法直接感知** —— ST7701 **没有状态回读脚**。所以"面板因为**供电跌落**掉了状态、而扩展器寄存器一个位都没变"那种黑屏，**本层看不见**（那要靠第 ③ 层：`r` 命令 + 复位原因 `BROWNOUT`）。本层查的是**因**，不是**果** |

### 13.9.3 频率、代价、以及"为什么不做周期性重初始化"

* **周期是自适应**（2026-09-25 凌晨改）：**稳态 2000 ms**，但一旦这一拍**读到坏值**
  （不一致）或**状态不明**（回读失败）⇒ 下一拍提前到 **200 ms**；修好之后
  **连续 3 个 200 ms 窗口**都干净 ⇒ 回稳态。
  ★ **这一条把"修好之后接着盯"的窗口从 2 秒压到 200ms** —— 而"一次被改写之后很可能
  又被改一次"，所以修完那一下才是真正要盯紧的时候。
  一次回读实测 **~400µs**（100kHz I2C，见 §13.8.4）⇒ 稳态摊到 2 秒里可以忽略；
  快速档只在"刚出过异常"的那几百毫秒里发生。
* ★ 最坏发现窗 = **当前周期**（稳态 2000ms / 快速档 200ms）—— 周期驱动的固有上限；
  实测**发现耗时 17ms**（注入排在检查前 40ms）与复检间隔 **190/201/200ms** 见 13.9.5。
* ★ **连续 3 次**检查都读到坏值 ⇒ 才**请求一次**自动重初始化，且之后有 **60 s 冷却期**。
  **不许**做成周期性重初始化 ✗ —— 那会让屏**定期闪**，而闪屏本身就是仪表盘上的故障现象。
  ★ **读失败不算"坏值"**：它只是"状态不明"（值得多盯两眼、进快速档），
  但**不进**连续异常计数 —— 否则总线一时不应答就会去反复重跑 41 步初始化。
* ★ 自动重初始化的**执行时刻**在 `dash_display_poll()` 开头（主循环里那个"可以慢一拍"的安全时刻），
  不在 I2C 对账那一刻 —— 重跑 41 步 + 重发一帧要 **208ms**（实测），不该把那一拍拉长。

### 13.9.4 日志字样（**判据就用这几行，改代码先改这一节**）

```
panelguard: exio rd=0x04 shadow=0x05 -> rewritten (t=15113ms)  ← ① 抓到了并按影子修回来了（带时刻）
panelguard: backlight rd=0 want=512 -> rewritten               ← ② 背光那一路（独立）
panelguard: exio rd err (no ack) -> retry next                 ← ③ 读失败（不算被改写）
panelguard: anomaly x3 -> auto reinit                          ← 连续 3 次读到坏值 ⇒ 请求一次自动重初始化
panel: reinit by guard (#1, 208ms; 41 步 + 重发 fb1; vsync 计数不回零)
panel: reinit by cmd r  (#1, 208ms; 41 步 + 重发 fb1; vsync 计数不回零)
inject: exio out 0x05 -> 0x04 (bit0, 影子保持 0x05, t=15096ms)  ← 注入行（**只有测试构建有**）
```

★ 前两行的 `(t=…)` 与注入行是**同一个 `millis()` 时基** ⇒ 直接相减就是**发现耗时**
（13.9.5 那些数字就是这么来的）。

诊断页第 1 页最后几行（**黑屏风险变成可看到的数据**）：

```
guard 84s rd=42 fix=1 bl=0 anom=1          ← ★ 本次开机（随重启归零）
sum rd=210 fix=3 bl=1 anom=4 k=7           ← ★★ 跨重启累计 + 落盘次数（2026-09-25 新增）
n=33 hb=7                                  ← 第几次上电/复位 + 心跳累计写过几次
```

`rd` 在涨 = 守护在跑（稳态每 2 秒 +1）；`fix` ≠ 0 = 真发生过"位被改写并且已经修回来"；
`bl` = 背光被重设的次数；`anom` = 读到坏值的次数（含已修的）。
★ `sum` 那一行与上面那行的**唯一区别就是"不随重启归零"** —— 它回答的是
"这块板**历史上**守护救过几回"（口径与心跳见 §13.9.8）；`k` = 累计值落盘过几次。

### 13.9.8 ★★ 跨重启留档：`prev=` / `prev_up=` / 守护累计（2026-09-25，"**事后能不能定性**"）

**起因（本单要解决的那个悬案，逐字记在案）**：累计启动次数在夜里 `31 → 32`（多了一次），
而串口上只看到**当前**这次开机的 `boot: reason=POWERON (raw=1)` —— **上一次的 reason 没有留档**
⇒ "那次到底是 USB 被断电（`POWERON`）还是板子自己掉电（`BROWNOUT`）"**判不出来**。

**做了三件事**（全部落在 NVS 命名空间 `dash`；判据层 `lib/dashcore/boot_persist.{h,cpp}`，
NVS 读写是**注入的回调**、时钟由调用方给 ⇒ 宿主机逐条钉着，13 例）：

| # | 留档 | 键 | 怎么来的 |
|---|---|---|---|
| ① | **上一次的复位原因**（名字 + 原始值） | `b_prevr` / `b_prevn` | 每次开机把**本次**的 reason 写下去 ⇒ 下一次开机读到的就是"上一次" |
| ② | **上一次运行了多久** | `b_hb_ms` / `b_up_ms` | ★ **心跳**：每 **10 分钟**把当前 uptime 写一次 NVS（掉电那一刻没人来得及记，只能运行期不断写） |
| ③ | **守护四个计数的累计** | `b_grd`/`b_gfix`/`b_gbl`/`b_ganm`/`b_gsnap` | 开机读回"上次为止的累计"当基线，搭心跳的车落盘（`b_gsnap` = 落盘过几次，跨重启单调 +1） |

**开机那一行**（真机原文见 §13.9.9）：

```
boot: reason=POWERON n=33 (raw=1) up=217ms | prev=BROWNOUT (raw=9) prev_up=27min | hb=3
                                                  ↑ 上一次的原因      ↑ 上一次跑了多久    ↑ 心跳累计次数
```

**怎么读（这一节就是给"事后看串口的人"写的）**：

| 看到什么 | 结论 |
|---|---|
| `prev=POWERON` | 上一次是**正常断电/插拔电**（人干的） |
| **`prev=BROWNOUT`** | ★★ 上一次**电压掉到 brownout 门限以下** —— 候选机制 B 的**直接证据**（芯片自己记的，不是推测） |
| `prev=PANIC` / `WDT` / `TASK_WDT` | 上一次是**崩了**（固件问题，不是供电） |
| **`prev_up=27min`**（大数） | 上一次是**长跑之后才断**的 ⇒ 正常 |
| ★ **`prev_up=3min`**（小数） | 上一次**跑几分钟就重启了** ⇒ **异常**（要么反复掉电、要么崩了 —— 配合 `prev=` 定性） |
| `prev=-` | NVS 里**还没有**上一次（全新板子 / 第一次跑带本层的固件）—— ★ **不是**故障 |
| `(nvs-)` 出现在 `n=` 旁边 | 这一次**没写进 NVS**（NVS 坏了）⇒ 下一次开机看不到这一次 —— ★ 与 `prev=-` 是**两件不同的事** |
| `hb=0` 且刚开机 | 正常（第一次心跳在开机 10 分钟后） |

**判据与代价（免得以后有人"顺手改成每分钟"）**：心跳 **10 分钟** = 144 次/天 ≈ **5.3 万次/年**，
NVS 有磨损均衡（日志式 + 页轮转）⇒ 可接受；**不要**做成每秒/每分钟 ✗（3100 万次/年，白耗寿命，
而心跳只回答"上一段大概多长"）。★ `prev_up` 的**分辨率就是心跳周期**、而且它是**下界**：
上一次开机后 **12 分 42 秒**被复位、而它最后一次心跳在 10 分整 ⇒ 下一次开机读到的就是
**`prev_up=10min`**（"**至少**跑了 10 分钟"）—— 真机实测就是这个数（§13.9.9）；
上一次跑 27 分钟读到 `27min`，是因为心跳恰好在 27 分那一格落过盘。
⇒ 够分"长跑 vs 几分钟"，**不够**做秒级计时。
★ 这一层是**单侧**判据：`prev=BROWNOUT` 能证实；它一次没出现**不能否定**机制 B
（电压可能只跌到面板掉状态、没跌到 ESP32 的门限）。

★ 心跳那一行（10 分钟一次，落盘成功才打）：

```
hb: up=601s rd=301 fix=1 bl=0 anom=2 k=7
```

★ 落地位置：`src/main.cpp` 的四个 `Preferences` 回调 + `boot_note()` + 主循环里那一次
`tick()`（**只在到点那一拍**真写 NVS，其余每一拍就是一次整数比较 ⇒ 不占显示时间线）。

### 13.9.5 ★★ 上板实测（COM6；含**故意注入故障**那一步）

**① 正常态（零改动）**：守护上线后**一行日志都没有**（`fix=0`），
`rgb:` 每秒那行照旧 `vsync +54/s … timeout=0`。⇒ "**一致 ⇒ 一次都不写**"在真机上成立。
...
| 项 | 结果 |
|---|---|
| **发现耗时** | `t=15096ms → t=15113ms` = **17 ms**（注入排在"下一次检查之前 40ms"，守护一读就撞上） |
| 日志原文 | `panelguard: exio rd=0x04 shadow=0x05 -> rewritten` ✓（逐字） |
| 快速档里的复检间隔 | 实测 **190 / 201 / 200 ms** ⇒ 确实是 **200ms 档**（不是 2 秒） |
| 屏 | 车主看到"闪一下" —— 坏值在总线上只存在 ~20ms（下一次检查就写回去了） |
| 与上一版对比 | 上一版**每次都要等慢周期（最坏 2 s）**；现在"刚出过异常"之后是 **200ms 一眼** |

★ 注入期间与恢复之后，「`rgb: vsync=… (+54/s) … timeout=0`」与每秒一行
`206 dash ok` **一秒都没断** —— 与"**驱动计数全正常而玻璃黑了**"那次事故的签名**逐字相同**
（那正是这条路径被认定是最可能诱因的理由）。

**③ ★ 自动重初始化（这一条以前只有宿主机证据，现在有上板证据了）**：
让守护"连续三次都读到坏值"（注入器改成"每一拍检查之前写一次坏值"，见 13.9.6），实测：
...
之后 `rgb: vsync=…(+54/s) … timeout=0` 与每秒一行 `206 dash ok` **一秒没断** ⇒
**屏自己回来了、停在表盘上**。整段（3 次坏值 + 1 次重初始化）挤在 **15.42s ~ 16.23s**（**0.8 秒**）。

★★ **写给"车主看到屏在闪"这件事**：那 0.8 秒里屏会**连续黑几下** —— 那是**刻意制造的故障**
（3 次写坏 + 1 次重跑初始化），**不是**反复发生的真实故障。正式固件里没有这条路径（13.9.6）。
...
`208ms` 是这条命令的**阻塞时长**（复位脉冲 10+50ms + 41 步里的 delay + CS 那两次 10ms）——
在仪表盘上这 0.2 秒是**可接受**的（对比"按物理 RST"要把整个 MCU 重启）。
★ 重初始化**之后** `vsync` 计数**不回零、`timeout=0` 照旧** ⇒ 没有把驱动/时序搞坏。

### 13.9.6 那条临时注入路径（**已经删掉**，留个复现的方子）

* 注入点：`src/dash_display_rgb.cpp` 的 `dash_panel_guard_fault_inject()`，门是
  **`-DPANEL_GUARD_FAULT_INJECT=1`**；串口命令是 **`i`**（可跟一位数字表示次数：
  `i1` / `i4`；判据在 `lib/dashcore/serial_cmd.h`）。
* 它把扩展器输出寄存器的 **EXIO1（= LCD_RST）那一位写反**，**故意不动影子** `g_exio_out`
  ⇒ "影子 vs 回读"立刻不一致 = 黑屏那条最可能的路径。
* ★★ **默认构建里这一段是空函数**（宏默认 0）：`i` 会被当成普通字符交回回放路径
  ⇒ 与"命令表里根本没有 `i`"**逐字节相同**。上板实测过一个**编译级**证据：
  带 `-DPANEL_GUARD_FAULT_INJECT=1` 的镜像 Flash = **908,659 B**（86.7%），
  去掉那一行之后 **908,143 B** —— 差 **516 B**，正是那一段注入代码本身。
* ★ 安全前提（复现时照做）：① 注入必须在**守护已生效**的固件上做；② 回滚镜像随手可用；
  ③ **一次只注入一串**、不许连续折腾；④ ~1 秒内没自愈 ⇒ **立刻烧回**并保留现场；
  ⑤ 测完把临时注入路径**删掉**（本次就是这么做的：`git diff platformio.ini` 为空）。

★★ **注入必须按守护的节拍走（两版失败的写法都记在这儿，别再用）**：
1. "发一次 `i` 写一次坏值" ⇒ 守护**只要检查一次就修好了** ⇒ 下一次检查读到的是干净的
   ⇒ `streak_` 清零 ⇒ **三连异常永远攒不够**（`anomaly x3` 一次都出不来）；
2. "按时间间隔（300ms）连打" ⇒ 守护发现第一次之后进**快速复检**（200ms 一眼）
   ⇒ 300ms 的间隔里必然夹着一到两次**干净读数** ⇒ `streak_` 还是被清零（实测两轮都失败）。
⇒ **正解**：让注入器**按守护的检查次数锁相**（门限 = "`dash_panel_guard_checks()` 变了"），
  **每一拍检查之前**写一次坏值 ⇒ 那一拍才会读到坏值 ⇒ `streak_` 才会 1→2→3。


### 13.9.7 顺手发现的一条"重初始化时不闪屏"的写法（**没有改驱动时序**）

本轮的 `dash_display_panel_reinit()` 刻意**只**做三件事：复位脉冲 → 重跑 41 步 → 重发当前 fb。
**不**重做 3 线 SPI 总线初始化、**不**重新申请 framebuffer、**不**重建面板对象与回调、
**不**改 PCLK/bounce/num_fbs —— 那些在 `dash_display_init()` 里已经做过，重做一次会
漏内存 / 重复注册回调 / 把换帧相位打乱。于是"重初始化"这一下的观感是**一次约 0.2 秒的
黑屏 + 画面原样回来**（车主实测：闪一下就回来了），而不是"整个 UI 重来一遍"。
★ 若哪天想把这一下也压掉（**纯光学**层面的做法：复位前把背光 LEDC 短暂拉到 0、
初始化完再恢复），那属于**独立一单**，而且要先量"背光开关本身对 3.3V 轨的冲击" ——
本轮**没有**做，也没有改任何时序。

### 13.9.9 ★★ 上板实测：心跳 / `prev=` / 累计（2026-09-25，COM6 原始行）

**判据与口径见 §13.9.8**；这里只贴**真机原文**（每一步都在同一块板上跑出来）：

```
① 第一次烧写后第一次开机（`prev_up=0min` ⇒ 上一次是"烧写那一下"起来的，只跑了几十秒）
boot: reason=POWERON n=36 (raw=1) up=217ms | prev=POWERON (raw=1) prev_up=0min | hb=0

② ★ 心跳真的落盘了（开机 600 秒那一刻，不是每个 tick）
hb: up=600s rd=302 fix=0 bl=0 anom=0 k=1

③ ★ 复位之后，`prev_up` 从 0 涨到 10 分钟（心跳 → NVS → 下一次开机读回，整条链闭环）
boot: reason=POWERON n=38 (raw=1) up=223ms | prev=POWERON (raw=1) prev_up=10min | hb=1

④ ★ 第二次会话的第二个心跳：`rd` 302 → 604（跨重启累计在涨）、`k` 1 → 2
...
| 判据 | 结果 |
|---|---|
| 心跳周期 | ✓ 实测 `hb: up=600s`（10 分钟那一刻才写） |
| 心跳落盘 ⇒ 下一次开机能看见 | ✓ `prev_up` **0min → 10min** |
| 累计跨重启 | ✓ `rd 302 → 604`、`k 1 → 2`、`hb 1 → 2` |
| ★ `prev_up` 的**分辨率**（如实记） | 第二次会话实际跑了 **12 分 42 秒**，报的是 **10min** ⇒ 它是**下界**（分辨率 = 心跳周期），这一条已写进 `boot_persist.h` 与 §13.9.8 |
| 显示链没被影响 | ✓ 心跳那一刻 `rgb: vsync +54/s … timeout=0`、每秒 `206 dash ok` 一秒没断 |

★ **`b` / `m` 两个命令照旧**（本单没碰蜂鸣器与静音那条路）：
...
★ **编译数字**（同一台机器、同一条 env）：基线 `134dd5b` = **908,143 B（86.6%）/ RAM 125,608 B**，
本单 = **912,491 B（87.0%）/ RAM 125,736 B**（**+4,348 B / +128 B**，余 ~133 KB）；
native **285 → 301 例**（2 skipped / 299 ok，0 失败）；JS 八套与基线逐项相同。
★ 本单在**中文路径**下直接编会失败（`ld.exe: cannot open map file … Invalid argument`）
⇒ 照 §14 的规矩用 **ASCII 副本**（`C:\206dash-scratch\Neru-sys`）编与烧。
...
| **`r`** | ★ **面板重初始化 + 重发当前画面**（2026-09-24 新增，**第 ③ 层防线**）—— 现场发现屏黑了、又不想按板上 RST 时的入口。它**不重启 MCU**、不动 PCLK/bounce/num_fbs（细节与判据见 §13.9） | `panel: reinit by cmd r (#1, 208ms; 41 步 + 重发 fb1; vsync 计数不回零)` |

★ **命令表本身已抽成纯函数** `lib/dashcore/serial_cmd.h` 的 `serial_cmd_classify()`
（判据层搬进 `lib/` 才编得进 native ⇒ 宿主机上可测，见
`test/test_dashcore/test_serial_cmd.cpp`）；"只认行首"那半条判据仍留在
`main.cpp` 的 `serial_cmd_handle()` 里（它依赖行缓冲与回放路径）。
...
★★ **2026-09-24 已获批准并上板**（决定与全部前后数字见 **§17.8**）。
...
1. **数字图集（任务书第 4 步）：没做** —— 理由是 flash 会涨且砍不掉对应字体（见 17.4），
   按任务书"停下来报"处理；★ 零 flash 的**替代做法已经获批并上板**（§17.8，16.2% → 3.8%）。
2. **"整表闪一刀"消没消、外弧还挡不挡数字：只有车主/朋友能判**（机上没有相机）。
   客观侧已经给到**板上**：静画/稳态的最大脏矩形从 **480×24** 收成 **~110×78**
   （§17.8 的真机原始行）⇒ "横跨整屏的读数带"在数字上已经不存在。
...
### 17.8 ★ 补（同日获批）：读数标签改成"**内容宽**" —— 上板数字（前后对比）

★ 这一条**不是**数字图集：**没有**新增任何数据/字体/渲染路径，只是把
`make_readout_label()` 里的 `lv_obj_set_width(l, LV_PCT(100))` **删掉**
（标签用 LV_SIZE_CONTENT），于是 `lv_label_set_text*()` 失效的范围从
"整屏宽 480"变成"**文字自己的外框**"。任务书的硬约束一条没破：
bounce / PCLK / `num_fbs` / 绘制缓冲 / 弧半径 / 251° 扫角 / 字号 / 字体 **全都没动** ✓。

**（a）pcpreview 2.8C 档 · 静画**（同一套统计口径，左右两屏合计）：

```
改前： inv=96867px2(=单屏的 42.0%) flush=23/s(累计172) fmax=480x78(单屏的 16.2%)
改后： inv=25751~27448px2(=单屏的 11.1~11.9%) flush=24~25/s fmax=115x78(单屏的 3.8%)
```

**（b）pcpreview · 扫表（头两秒）**：

```
改前： inv=884259px2(383.7%) fmax=480x480(100.0%)  → inv=576098px2(250.0%) fmax=147x332(21.1%)
改后： inv=884259px2(383.7%) fmax=480x480(100.0%)  → inv=426487px2(185.1%) fmax=147x332(21.1%)
```
（第 1 秒**逐字节相同** —— 那是开机首帧的整屏渲染，与读数无关；第 2 秒的差就是读数带那部分。）

**（c）真机 COM6 · 稳态（"静画"档）**：

```
改前： rgb: 脏区/s inv=199663px2(=一屏的 86.6%) flush=31/s(累计214) fmax=480x24(一屏的 5.0%)
改后： rgb: 脏区/s inv=55916px2(=一屏的 24.2%) flush=16/s(累计149) fmax=111x78(一屏的 3.7%)
      rgb: 脏区/s inv=52867px2(=一屏的 22.9%) flush=16/s(累计165) fmax=106x78(一屏的 3.5%)
      rgb: 脏区/s inv=51037px2(=一屏的 22.1%) flush=16/s(累计181) fmax=109x78(一屏的 3.6%)
```

**（d）真机 · 扫表（头两秒）**：

```
改前： inv=234876px2(101.9%, fullrb=1/s) fmax=480x24  → inv=468524px2(203.3%) flush=89/s fmax=240x48
改后： inv=234876px2(101.9%, fullrb=1/s) fmax=480x24  → inv=412912px2(179.2%) flush=84/s fmax=90x128
```
...
| 判据 | 结果 |
|---|---|
| 真机静画 `inv` 从 86.6% 明显降下来 | **86.6% → 20.4~26.1%**（≈3.8 倍） ✓ |
| 真机静画 `fmax` 从 480×24 变小 | **480×24(5.0%) → 105~124×78(3.1~4.1%)**，**宽度从 480 变 ~110** ✓ |
| flash 仍 ≤ 905,059 B | **905,051 B（−8 B）** ✓ |
| RAM 不变 | **125,512 B** ✓ |
| 读数位置/压弧一个像素没动 | 落帧实测墨迹 `x[181..296] y[71..104]`、**压弧像素仍为 0** ✓ |
| 回归 | native **266（2 skipped / 264 ok）**、JS 八套同基线 ✓ |

---

## 18. 2026-09-26 日志那一行**不再阻塞主循环**（"装车即废"那条拦路虎）

> 起因：`ARCHITECTURE.md` §7.5.6 把根因量出来了 —— **没人读 `COM8` 时主循环 28 秒只转 2 圈**
> （`n=1 in 20120ms` + `stalled 7689ms`），而**车上的常态就是没人读**。
> 修法与硬约定写在 `ARCHITECTURE.md` **§7.5.7**；本节只放**这一块板上的实测数字**。

### 18.1 这一条为什么先做它

车主**会开车上路**。只要"没人读串口 ⇒ 主循环卡住"这条成立，那么装到车上
（USB 主机永远不在）就是"画面冻结 + 蜂鸣器长鸣"。
所以本轮的第一目标只有一个：**让主循环永不因日志阻塞**，其它一律不碰
（PCLK / bounce / `num_fbs` / 驱动时序 / 抓帧盒 env / 17-18 / GPIO15 **全部没动**）。

### 18.2 改了什么（三个数）

| 项 | 值 | 出处 |
|---|---|---|
| 日志环 | **8192 B** 静态内存（不碰堆） | `lib/dashcore/dash_log.h` 的 `DASHLOG_RING_BYTES` |
| 每圈排空预算 | **512 B** | `DASHLOG_DRAIN_BUDGET`（与链路收帧那条 `kLinkRxBytesPerLoop` 同一个理由） |
| 高频那几行的节流 | **2 秒一条**（原 1 秒） | `206 dash ok`（`src/dash_ui.cpp` 的 `g_ok_gate`）、两行 `rgb:`（`src/dash_display_rgb.cpp` 的 `rgb_log_gate`） |

★ 节流**只挡打印，不挡统计**：`rgb:` 那两行里的数字仍然是**每秒**的口径
（`flush_stats_reset` 与 `last_*` 照旧每秒推进一次）—— 这一条是硬的，
否则读数会变成"两秒攒一起"，看着像吞吐翻倍。

### 18.3 摘要行多了七个格子（判据就在这里）

```
loop: n=… in …ms (…/s) max=…ms stall=… | link rx bytes=… frames=… | log drain=… drop=… dropped=… blocked=… ring=…/… hwm=…
```

| 格子 | 含义 | 怎么读 |
|---|---|---|
| `drain=` | 累计交付给串口的**字节** | 差值 = 这一窗口真的送出去多少 |
| `drop=` / `dropped=` | 累计丢弃的**整行数** / **字节数**（环满） | **不是 0 也没关系** —— 这是"宁可丢日志也不堵主循环"这条取舍的读数 |
| `blocked=` | 累计"端口报满 ⇒ 这一圈一个字节都没写出去"的**次数** | ★ 它在涨 = **保护在工作**（这正是"没撞上驱动超时"的证据） |
| `ring=`/`hwm=` | 此刻欠着多少 / 环最满到过多少 | `hwm=8189`（≈ 8192）就是"环填满过" |

### 18.4 上板实测（`COM8`，从板镜像 `esp32s3-rgb-slave`，三段一次跑完）

**做法**：一个脚本**只开一次口**，自己控制"读 → 关 → 再读"三段
（开/关串口本身会让这块板堵十几秒 —— 上一单实测 8003 / 14071 ms，
所以"少开口"是纪律，不是洁癖）。原文：`C:\206dash-scratch\logfix\triphase2.raw`。

```
[21:29:34] A: opened COM8 -- host IS reading
...
**② 没人读 90 秒（B 段）**：这一段**串口上一个字都收不到**（口关着，天经地义）。
判据只能从**设备自己的时钟与计数器**读 —— 它们是**逐毫秒对上**的：

| 读数 | A 段末尾 | C 段开头 | 差值 | 说明 |
|---|---|---|---|---|
| `link: sim tick_age=` | **110000 ms** | **205000 ms** | **+95000 ms** | 设备时钟在关口的 90 秒里**一直在走**（`tick_age = now - 最后一次 TICK`，sim 档就是墙钟） |
| `beeps=`（蜂鸣器计数） | **27** | **49** | **+22** | 这 90 秒里告警层与蜂鸣器**照常响过 22 次**（屏上红区一直在跑） |
| `rgb: vsync=N(+54/s)` | 5419 | 10915（C 段头一行） | **+5496 帧** | 面板一直按 54 帧/秒在换帧（**画面没冻**） |
| `buzz safety=`（2 秒绝对上限被掐次数） | **0** | **0** | **0** | ★★ **"长鸣"没有发生**（§7.5.3 那条兜底一次都没用上） |
| `log drain=` | 40542 | 51246（C 段第一行） | +10704 | 环里积压的 8 KB 在**开口后几秒内排完** |
...
★ **`drop` 从 71 涨到 307、`dropped` 从 8081 涨到 34862**（+26,781 B），
而 `drain` 只涨了 10,704 B ⇒ 这一窗口里日志**总共产生 ≈ 37 KB**（10.7 + 26.8），
而 8 KB 的环装不下 ⇒ **多出来的 26.8 KB 被丢了**（而不是"让主循环等"）。
**这就是设计要的行为**：丢日志、不堵循环。
...
| 判据 | 结果 |
|---|---|
| 有人读时主循环正常 | **20,912 / 21,757 圈/秒，`stall=0`，`max=32~85ms`** ✓ |
| **没人读 90 秒 ⇒ 主循环照常**（上一单这里是"28 秒只转 2 圈"） | **`stall=0`、`max=97ms`**；设备时钟 +95000 ms、蜂鸣器 +22 次、面板 +5496 帧 ✓ |
| 再打开立刻出数据、无长停顿 | C 段**每一行都是 `stall=0`**，第一行就 19,066 圈/秒；环里 8 KB 几秒内排完 ✓ |
| 蜂鸣器不会长鸣 | **`buzz safety=0` 全程**（那 90 秒里一次都没被掐）✓ |
| 丢字节可观测 | **`drop=307 dropped=34862`**（三个格子都在摘要行里）✓ |
| 日志**绝不**阻塞主循环 | `blocked=` 在涨（2,255,722 → 2,794,642）= 一路上都在"端口报满就一个字节都不写" ✓ |

★ **`max` 的量级没变**（31~97 ms，与 §7.5.4 记的"合法长圈 141~225 ms"是同一档）
⇒ 日志那一行**不再是长圈的来源**。

### 18.6 老实说的边界

1. **"没人读"那 90 秒的直接串口原文是不存在的**（口关着，物理上收不到）。
   上面 ② 那一格是**从设备自己的时钟与计数器反推**的（`tick_age` 逐毫秒对上），
   与上一单定位根因用的是**同一套算法**（那一次也正是靠它算出来的）。
2. **B 段没有 `drop` 增长**（只在 C 段第一窗口里看到）：说明**没人读时环并没有一直涨** ——
   设备会很快判定"端口写不出去"，然后**一个字节都不写**（`blocked` 就是这么涨的）。
   真正把环填满的是**刚开口那几秒**（主机在收、但设备侧一波补投 + 节流前的积压）。
   ⇒ "环满"只发生在**过渡**那一瞬间，稳态下（无论有没有人读）环都是空的。
3. **`hwm=8189` 而不是 8192**：`Ring` 的容量口径是"整段放得下才写"，
   8192 的环在填到剩 3 B 时就再也放不下任何一行（最短的一行也比 3 B 长）⇒ 高水位停在 8189。
4. **没有量"单次 `Serial.write()` 的耗时"**（与上一单同一个理由：造不出"有人读的同时没人读"）。
   `blocked=` 是这条的**间接读数**：它涨 = 那些圈**压根没进 `write()`**。
