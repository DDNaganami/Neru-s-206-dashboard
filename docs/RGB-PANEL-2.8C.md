...
> 并新增一行"编译期事实"把"到底几字节"交给**编译器**回答：
>
> ```
> rgb: 编译期事实 LV_COLOR_DEPTH=16 sizeof(lv_color_t)=3 sizeof(draw_buf)=23040 元素行=16 flush行上限=24(每行960B)
> rgb: 脏区单笔上限=24行(23040B) —— bounce 两块共18KB(单笔比它大,旧口径不成立) 以实测 blit_max 为准
> ```
>
> ★★ **这行新口径随"下次烧写"生效**：本轮**没有**为了它单独烧板（它只是两行日志文本），
