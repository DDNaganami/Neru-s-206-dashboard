...
* 第一轮（只有 ①② 的固件）车主反馈：**① 听到"哔"5 声 ✓**、**② 出声且音高随频率变化 ✓**；
  ★ 但按 13.2 的论证，这两条对"有源 / 无源"**两种解释都相容** ⇒ **不能据此定性**。
* 本轮（带 ③④ 的固件）：③ **持续拉高 2 秒** 与 ④ **持续拉低 2 秒**（对照）—— 请车主各听一次并回一句话：

| ③ 听到什么 | 结论 | ④ 应当 |
|---|---|---|
| **一整段不间断的稳定音**（2 秒一个音） | **有源**（自带振荡，给直流就响） | 完全静音 |
| 只有起止两声"咔哒"、**中间是静音** | **无源**（要外部给频率才响） | 完全静音 |

* 顺带记一下**音量/音色**（大声/小声、清脆/沙哑）—— 它决定告警音该定多长、夜里会不会太吵。
* 结论落在这一节与 `ACCEPTANCE.md` 的同日条目里；**在车主回话之前，这里不写"已裁决"**
  （`buzzer.h` 里 L14 的措辞也是"建议，不是已裁决"）。

### 13.7 接真机那一档：**只写方案，不实现**（等车主点头）

**若 ③ = 不间断稳定音（有源）** —— 需要的代码量极小，且 `alerts` 一行都不用改：
