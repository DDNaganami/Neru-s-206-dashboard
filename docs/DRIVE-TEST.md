...
★★ **但开机头 ≤15 秒是例外，别在那一段判链路死**（2026-09-27 深夜新增的启动闸门）：
主板现在**先让 BLE 建连、再启链路**（见 §6.7），所以上电后那几秒从板**本来就没数据**
（`vanraw: age` / `tick_age` 在涨、屏上是模拟数据 + 角标）。判据是那两行：
- `link: 启动闸门开了 —— …` ⇒ 从这一刻起才开始有数据，**之前的空窗不算故障**；
- 若一直没看到这一行、`vanraw: age` 持续涨过 **20 秒** ⇒ 那才是真问题（闸门没开）。
