...
//     丢帧、水位、统计全在同一处看得见。
//
// ★★ 链路 UART 必须**独立于日志口**（§0 的待办）
//   §0「载体」那条：链路用的就是 UART0 那一对脚（43/44）⇒ 显示构建必须**关掉 UART0
//   文本日志**，日志只走原生 USB-CDC，否则 `dash_logf()` 的文本会**混进链路数据流**。
//   本文件把这件事在**代码里钉住**：`kLinkUartPort == 0`（链路就是 UART0 /
//   Serial0）时，`LINK_PHY_UART_ALLOW_LOG_ON_UART0` 必须是 1 才编得过 ——
//   而 `dash_log.h` 的 `dash_log_begin()` / `dash_logf()` 里**硬调了 `Serial0`**，
//   所以把它们链进来就等于日志也在往链路那根线上写。这两条编译期守卫合起来 =
//   "日志与链路不能同用一个外设"，上板时撞上就是**编译错误**，不是静默的坏帧。
//   ★ 本轮**没有改 `dash_log.h`**（口径是"别去改它"）：那条待办仍然是待办，
//     只是现在有了一道编译期闸门看着它。
//
...
//   "链路占 UART0 ⇒ 日志不许也写 UART0" 这条要成立，判据是"**这份固件里有谁在调
//   `dash_logf()`**"—— 那件事只有 link_phy_uart.cpp 知道（它是同时 include 了
//   `dash_log.h` 与链路口径的那一个翻译单元，也正是"链路 PHY 被链进固件"这件事的
//   唯一证据）。所以：
//     · 本头文件（谁都能 include）**只给常量与纪律**，不替别人下结论；
//     · 闸门在 .cpp：链路 PHY 一进固件、而 UART0 上还挂着日志 ⇒ **编译期直接报错**，
//       附一条 escape hatch（-DLINK_PHY_UART_ALLOW_LOG_ON_UART0=1）与两条出路。
//   ★ 本轮**没有改 `dash_log.h`**（口径是"别去改它"）：§0 那条待办仍然是待办，
//     只是现在有一道编译期闸门看着它。
//
