...
void LinkPhyUart::start(HardwareSerial* s, int8_t port, int8_t tx, int8_t rx, bool loopback) {
  ...
  // ============================================================
  // ★★ RX 脚**上拉** —— 2026-09-25 新增（跟着"屏卡死 + 蜂鸣器长鸣"那一单一起进来）
  //
  // 为什么必须有（这不是防御性代码，是对着一个真实症状加的）：
  //   · 契约 §0 的接线是"每块板 43 发、44 收"，而**4Pin 那一排平时是空着的**
  //     （台面上只有 Type-C 供电那根线）。§8 L1 记的那颗 `FSUSB42UMX` 是"二选一"
  //     的模拟开关 —— 插着 Type-C 时它把 4Pin 那一路**断开** ⇒ **GPIO44 悬空**。
  //   · 悬空输入 + 旁边就是 15 MHz 的 RGB 并口（16 根数据线 + PCLK 每帧翻 480×480 次）
  //     ⇒ 44 脚上是一条**持续的伪字节流**（浮空 CMOS 输入被邻线耦合灌进来）。
  //     从板那一侧就会变成"每圈都有几百个字节要处理、一帧都解不出来"。
  //   · 上拉把它**钉在高电平**（UART 空闲态就是高）⇒ 悬空时线路静止、`read()` 立刻返回 -1。
  //
  // ★ 为什么用 `pinMode(rx, INPUT_PULLUP)` 而**不是** `gpio_set_pull_mode`：
  //   本文件在宿主机上编不了（`HardwareSerial` 是 Arduino 的东西），而这份代码
  //   **只在 Arduino 固件里**编 —— 用 Arduino 的 API 最短、可读，也不用把
  //   `driver/gpio.h` 再拉进来。
  //
  // ★★ 为什么**只配 RX、绝不碰 TX**（`ARCHITECTURE §8 L1` 的残余 ⓒ 就是这条）：
  //   `pinMode(rx, INPUT_PULLUP)` 只动 rx 那一根。**不许**对 43/44 写
  //   `OUTPUT`/`INPUT_PULLDOWN`：43 是 TX（UART 外设自己驱动它，把它改成普通输出
  //   就等于把 UART 的推挽和 GPIO 的推挽叠在同一个脚上）；44 一旦被配成输出，
  //   这条线就会**主动驱动总线**（与主板对打）。
  //   ⇒ 这里只有一行、只有 rx、只有 INPUT_PULLUP。
  //
  // ★ 顺序：放在 `s->begin()` **之后**。UART 驱动装好之后这个脚才是"UART 的 RX"，
  //   而我们只改它的**上拉**（`gpio_pullup_en`），不动它的功能选择（那是 matrix 的事）
  //   ⇒ 不影响 UART 收字节。放前面也能work，但放在后面语义更直白：
  //   "线路已经由 UART 接管了，我给它一个确定的空闲电平。"
  //   ★ 上拉强度 ~45 kΩ，对 115200 的边沿（≈8.7 µs/位）**不构成负担**：真接了主板时
  //     推挽驱动会直接盖过它（这是标准做法，不是折中）。
  // ============================================================
  if (rx >= 0) {
    pinMode((uint8_t)rx, INPUT_PULLUP);
  }
