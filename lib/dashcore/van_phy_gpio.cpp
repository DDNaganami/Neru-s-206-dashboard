...
// 总线空闲多久算"这一帧结束"。
//
// 依据:van_wire 的 EOF 判据是"连续 8 个 recessive 槽" = **66µs**
// (8 × van_wire.h 的 kTsNs 8250ns = 8.25µs/槽,实车实测值;
//  规范的 125kbit/s ⟹ 8.00µs 只是标称,按它算只有 64µs)。
// 所以只要空闲超过 66µs 就说明帧尾已经到了;这里取 300µs 留足余量 ——
// ★ 300µs 是**固定值,不随时基缩放**:它只是"判晚一点"的余量,与槽时间无关
