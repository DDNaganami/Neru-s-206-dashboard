...
//
// ---- 验证到什么程度(别把"能解析"读成"画对了") ----
//   · 格式 / 解析 / 取像素:宿主机单测 + JS↔固件**逐字节往返对账**
//   · 画到屏上:pcpreview 用真 LVGL 渲成帧,逐像素验过图层顺序与透明
//     (tools/theme-editor/check-preview-frame.js)
//   · 真屏:240×240 那块验证板(微雪双屏)的两块圆屏已经在出画面;
//     图片资源的验收步骤见 README 的「微雪双屏 240×240」第 5 节
//   · 480 那块 2.8" 屏的驱动时序仍没在真屏上验过(见 README「范围与限制」)
// ============================================================
...
// ------------------------------------------------------------
//   · src/image_load.cpp:setup() 里调一次 image_load(),拿到 header 与像素指针
//   · src/dash_ui.cpp:dash_ui_init() 先探测每个角色有没有图,有图就
//       lv_image_create + lv_obj_center —— 创建顺序就是图层顺序:
//       背景铺最底 → 弧线压在它上面 → 表情在最上层 → 数字读数最后建;
//       一张图都没有就退回程序化形状表情 + 主题纯色背景(降级,不是错误)
//   · ★ **图是原尺寸居中的**:固件里没有 lv_image_set_scale,所以
//     "图像的像素尺寸 = 屏上的尺寸"。这正是 240×240 那块板表情要用
//     152(上限 160、内圈副弧内沿半径 81)的原因 —— 见
//     tools/theme-editor/image-blob-build.js 的 FACE_SIZE_TIERS,
//     以及 tools/theme-editor/README.md 的「微雪双屏 240×240」一节。
//   · 换图只刷 image 分区(0x254000),固件一个字节都不动 ——
//     镜像在 setup() 里 mmap 一次,所以刷完要**重启**才看得到。
// ============================================================
